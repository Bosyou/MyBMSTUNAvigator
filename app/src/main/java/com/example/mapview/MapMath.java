...
import com.example.mapview.math.FloydAlgorithm;
import com.example.mapview.math.GeneticAlgorithm;
import com.example.mapview.math.TSPNearestNeighbour;
...
public final class MapMath {
    ...
    public static float getDistanceBetweenTwoPoints(PointF start, PointF end) {
        return (float) Math.sqrt(Math.pow(Math.abs(end.x - start.x), 2)
